...
type SkateboardPhysicsProps = {
  boardTextureURLs: string[];
  className?: string;
};

export function SkateboardPhysics({
  boardTextureURLs = [],
  className,
}: SkateboardPhysicsProps) {
  ...
  useEffect(() => {
    ...
    function createBoundaries(width: number, height: number) {
      const size = 20;
      return [
        Bodies.rectangle(width / 2, -10, width, size, { isStatic: true }), // Top
        Bodies.rectangle(-10, height / 2, size, height, { isStatic: true }), // Left
        Bodies.rectangle(width / 2, height + 10, width, size, {
          isStatic: true,
        }), // Bottom
        Bodies.rectangle(width + 10, height / 2, size, height, {
          isStatic: true,
        }), // Right
      ];
    }
